...
def test_image_data():
    ...
    list_data = [1, 20, 3, 4, 5, 10]
    image.data.extend(list_data)
    print(f"     Assigned new data: {list(image.data)}")
    assert len(image.data) == 6, f"Data length should be 6, got {len(image.data)}"
    for i, expected in enumerate(list_data):
        assert (
            image.data[i] == expected
        ), f"Data[{i}] should be {expected}, got {image.data[i]}"
    print("     ✓ Direct data assignment test passed")

    assert image.data[5] == 10, f"Data[5] should be 10, got {image.data[5]}"

    # Test array indexing
    print("   Testing array indexing:")
    assert image.data[0] == 1, f"Data[0] should be 1, got {image.data[0]}"
    assert image.data[5] == 10, f"Data[5] should be 10, got {image.data[5]}"
    print("     ✓ Array indexing test passed")

    # Test array iteration
    print("   Testing array iteration:")
    data_sum = sum(image.data)
    expected_sum = sum(list_data)
    assert (
