...
def test_joint_command():
    ...
    for i in range(3):
        ...
        print(f"     Added joint {i}: operation_mode={single_joint.operation_mode}, pos={single_joint.pos}")
...
def test_joint_state():
    ...
    for i in range(3):
        ...
        print(f"     Added joint state {i}: status_word={single_state.status_word}, posH={single_state.posH}")
...
def test_joint_structures_comprehensive():
    ...
    for i, (status, posH, posL, vel, toq, current, err) in enumerate(state_configs):
        ...
        print(f"     State {i}: status={status}, posH={posH}, vel={vel}, current={current}")
...
def main():
    ...
    try:
        ...
        print("\n" + "=" * 60)
        print("🎉 All JointState and JointCommand binding tests completed successfully!")
        print("\nSummary:")
        print("  ✓ SingleJointCommand - operation_mode, pos, vel, toq, kp, kd")
        print("  ✓ JointCommand - timestamp, joints array")
        print("  ✓ SingleJointState - status_word, posH, posL, vel, toq, current, err_code")
